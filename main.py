...
@app.post("/api/refresh_reports")
async def refresh_reports():
    """Refresh all surf spot data"""
    from surf_report import SurfReportGenerator
    
    try:
        generator = SurfReportGenerator()
        results = generator.update_all_spots()
        return results
    except Exception as e:
        logging.error(f"Error refreshing reports: {e}")
        return {
            "status": "error",
            "updated_spots": [],
            "errors": [str(e)],
